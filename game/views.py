...
class NewGameView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({'error': 'Failed generate level1: ' + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        LevelData.objects.create(game=game, level_number=1, role=level_content.get('role', 'detective'), content=level_content)
        level_summary = next((lvl['summary'] for lvl in outline.get('levels', []) if lvl.get('level_number') == 1), '')
        return Response({'game_id': str(game.pk), 'level': level_content, 'level_summary': level_summary})
...
class NextLevelView(APIView):
    def post(self, request):
        ...
        if not game_id or choices_path is None:
            return Response({'error': 'game_id and choices_path required'}, status=status.HTTP_400_BAD_REQUEST)
        try:
        ...
        try:
            ...
            return Response({'error': f'Failed generate level{next_level}: ' + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        game.current_level = next_level
        game.save()
        LevelData.objects.create(game=game, level_number=next_level, role=level_content.get('role', ''), content=level_content)
        level_summary = next((lvl['summary'] for lvl in game.outline.get('levels', []) if lvl.get('level_number') == next_level), '')
...
class HeadlineView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({'error': 'Failed generate headline: ' + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class GenerateBackgroundView(APIView):
    def post(self, request):
        ...
        if level_number is None or level_summary is None:
            return Response({'error': 'level_number and level_summary required'}, status=status.HTTP_400_BAD_REQUEST)
        # Fallback defaults
        ...
        try:
            bg_info = generate_dynamic_background_prompt(level_number, level_summary, node_context=scene_description)
            prompt = bg_info.get('prompt')
            image_name = bg_info.get('image_name')
            success, err = generate_and_save_image(prompt, image_name, is_background=True)
            if not success:
                url_path = f'/static/images/{default_image}'
                return Response({'prompt': None, 'image_name': default_image, 'url': url_path})
...
class GenerateSpriteView(APIView):
    def post(self, request):
        ...
        if not character_name or not character_description:
            return Response({'error': 'character_name and character_description required'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            sp = generate_dynamic_sprite_prompt(character_name, character_description)
            prompt = sp.get('prompt')
            image_name = sp.get('image_name')
            success, err = generate_and_save_image(prompt, image_name, is_background=False)
            if not success:
                return Response({'error': 'Sprite gen failed: ' + err}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            url_path = f'/static/images/{image_name}'
            return Response({'prompt': prompt, 'image_name': image_name, 'url': url_path})
        except Exception as e:
            return Response({'error': 'Sprite prompt/gen failed: ' + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)